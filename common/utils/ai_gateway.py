...
class OpenaiClient:
    ...
    def generate_sql_by_openai(self, db_type: str, table_schema: str, user_input: str, sample_data: str = ""):
        ...
        # prompt 含 DDL/样本数据等潜在敏感内容：INFO 只记长度，全文仅 debug 可见
        logger.info("NL2SQL 请求，prompt 长度: %s 字符, 模型: %s", len(prompt), self.default_chat_model)
        logger.debug("NL2SQL prompt 内容: %s", messages)
        try:
    ...
    def optimize_sql_by_openai(
        ...
        messages = [dict(role="user", content=prompt)]
        logger.info("SQL 优化请求，prompt 长度: %s 字符, 模型: %s", len(prompt), self.default_chat_model)
        logger.debug("SQL 优化 prompt 内容: %s", messages)
        try:
            res = self.request_chat_completion(messages)
            return res.choices[0].message.content
        except Exception as e:
